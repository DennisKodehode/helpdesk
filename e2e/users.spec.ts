...
test.describe
  .serial("Users page — CRUD", () => {
    ...
    test("admin can create a new agent who then appears in the table", async ({
      page,
    }) => {
      // Open the dialog.
      ...
      // RoleBadge renders "Agent" (capitalised) inside a styled span.
    ...
    test("admin can edit the created agent and the table reflects the change", async ({
      page,
    }) => {
    ...
    test("admin can delete the created agent and they disappear from the table", async ({
      page,
    }) => {
      ...
      await expect(
        confirmDialog.getByRole("heading", { name: "Delete agent?" }),
      ).toBeVisible();
...
test.describe
  .serial("Users page — integration", () => {
    // =========================================================================
    // 1. Password change works end-to-end
    // =========================================================================

    test("changed password is enforced: old password rejected, new password accepted", async ({
      page,
    }) => {
      const AGENT = {
        name: "PwChange Agent",
