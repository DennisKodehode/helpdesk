...
| AI       | Google Gemini via Vercel AI SDK (`@ai-sdk/google`, model `gemini-2.5-flash-lite`) — auto-categorization, auto-resolution, agent-facing **Polish** (KB-grounded review of the agent's own draft: fact-checks against the category-filtered KB, returns confidence + change summary + cited sources; the agent's *decision* stands, the KB corrects *facts*), summaries, and **KB gap-analysis** (proposes new articles from resolved tickets). Auto-resolve draft logic in `server/src/lib/draft-reply.ts`; Polish lives in the `polish-reply` route |
...
| `server/prisma/schema.prisma` | Full schema: User, Session, Account, Verification, Ticket, Reply, KbArticle, KbSuggestion |
| `server/prisma/seed.ts` | Seeds the admin user + AI user + SLA policies + KB articles (`bun run db:seed` from `server/`) |
| `server/src/lib/kb-corpus.ts` | **KB retrieval seam** — `getRelevantArticles(category)` assembles the AI corpus from published articles (category-filtered). The one function a future pgvector upgrade replaces |
| `server/src/lib/kb-gap-analysis.ts` | Daily cron: scans resolved tickets for recurring uncovered topics → pending `KbSuggestion`s for admin review |
| `server/src/routes/kb-articles.ts` / `kb-suggestions.ts` | Admin KB CRUD; suggestion approval queue (list/count/approve/reject) |
| `client/src/pages/KnowledgeBasePage.tsx` | Admin **Knowledge base** screen (`/knowledge-base`): Articles tab + Suggestions approval tab |
...
- **Knowledge base**: DB-backed structured articles (`KbArticle`), **not** the old `server/knowledge-base.md` blob — that file now only seeds the initial rows (`server/src/lib/kb-seed.ts`). The AI corpus is assembled per-request from *published* articles, filtered by the ticket's classified category, through the single seam `getRelevantArticles(category)` in `server/src/lib/kb-corpus.ts` (a future pgvector upgrade replaces only that function). Admins manage articles on `/knowledge-base`. **Self-grow**: a daily cron (`kb-gap-analysis.ts`) proposes articles from recurring resolved tickets as pending `KbSuggestion`s; admins approve (→ creates a published `KbArticle`) or reject on the Suggestions tab; agents can file one via "Suggest for KB" on a resolved ticket. Nothing auto-publishes — admin approval is the security boundary (ticket text is attacker-controllable and wrapped as untrusted in every drafting prompt). Cadence is admin-configurable via `workflow_settings` (`kbGrowthOn`/`kbGrowthIntervalDays`/`kbMinClusterSize`); the cron runs daily and gates on `kbGrowthLastRunAt`.
