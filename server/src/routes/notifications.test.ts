...
describe("notifications API", () => {
  ...
  beforeAll(async () => {
    ...
    const signInRes = await request(app)
      ...
      .send({ email: "notif-recipient@example.com", password: "Testpassword1!" });
    ...
    const ticket = await prisma.ticket.create({
      data: {
        fromName: "Customer",
        fromEmail: "customer-notif@example.com",
        subject: "Notif test ticket",
        body: "Body",
  ...
  afterAll(async () => {
    await prisma.notification.deleteMany({
      where: { OR: [{ userId: recipientId }, { userId: otherUserId }] },
    });
    ...
    await prisma.user.deleteMany({
      where: { id: { in: [recipientId, actorId, otherUserId] } },
    });
  });
