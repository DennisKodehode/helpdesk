...
router.post("/", async (req, res) => {
  ...
  const emailResult = await resend.emails.receiving.get(event.data.email_id).catch((err) => {
    console.error("[inbound-email] Could not retrieve full email body:", err);
    return null;
  });

  const bodyText = emailResult?.data?.text ?? undefined;
  const bodyHtml = emailResult?.data?.html ?? undefined;
